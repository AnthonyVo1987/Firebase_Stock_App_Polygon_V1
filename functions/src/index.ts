...
export const getStockDataAndInsights = onCall(
  ...
  // Log function call with structured data
    ...
    // to fetch stock data from Polygon.io
      ...
      // Craft a detailed prompt for the AI to generate relevant stock insights
        const prompt =
      "Generate three simple insights based on the following stock data " +
      `in a concise list format:\\n${JSON.stringify(stockQuote)}. Focus on ` +
      "key metrics like price change, volume, and daily range.";
